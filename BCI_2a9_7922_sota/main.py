"""
Author: Yici Liu
Date: 2026-04-22 15:36
Q-Mixer for MI Classification: Nine subjects from the BNCI20140001 dataset, LOSO evaluation, pre-training + fine-tuning paradigm.
"""
    ...
    def train(self, fold):
        ...
        for e in range(self.n_epochs):
            ...
            for i, data in enumerate(train_dataset):
                x_src = [data[f"Sx{idx}"] for idx in range(1, 9)]  # 14 个域
                y_src = [data[f"Sy{idx}"] for idx in range(1, 9)]
                ...
                domain_label = torch.arange(8, device=device, dtype=torch.long).repeat_interleave(B)
                ...
                tok_s = tok.view(8, B, -1)      # [14, B, feat]
                lab_s = label.view(8, B)        # [14, B]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--d_classes', type=int, default=8)
