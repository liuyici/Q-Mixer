...
class ExGAN():
    def __init__(self, args, nsub, fold):
        ...
        self.model = QuantGate(emb_size=7021, depth=6, bottleneck_dim=128, n_classes=2).float().cuda()
        self.domain_Discriminator = Discriminator(emb_size=7021, n_classes=4).to(self.device).float()
    ...
    def get_source_data(self, feature="de_LDS"):
        if self.args.dataset == "seed":
            datasets, dataset_test, X_subjects, Y_subjects = load_mi1(args, path=r"E:/Research/EEGDataSet/BNCI20140mi5/Rawdata/saved_loso_W1_logmap_gfk", n_windows=1, k=25)
...
def main(args):
    ...
    print("\n================= Fine-tuning平均结果 =================")
    print(f"每个微调iter在59个受试者上的平均准确率: {mean_ft_acc}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_iter2', type=int, default=102)
    ...
    parser.add_argument('--file_path', type=str, default="E:\Research\EEGDataSet\BNCI20140mi5\Rawdata\saved_loso_W1_logmap_gfk", help="Path from the current dataset")
